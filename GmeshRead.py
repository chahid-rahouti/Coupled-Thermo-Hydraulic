...
class mesh2d:
    ...
    # Function to take the border points by name 
    def get_border_points_by_name(self, border_name):
        # 1. Trouver l'ID physique associé au nom
        physical_id = None
        in_phys = False
        for line in self.lines:
            ...
            if in_phys:
                ...
                if len(parts) >= 3:
                    ...
                    if dim == 1 and name == border_name:
                        physical_id = int(phys_id)
                        break
        if physical_id is None:
            print(f"Nom de bord '{border_name}' non trouvé.")
            return []

        # 2. Trouver les lignes (éléments type 1) associées à cet ID physique
        in_elem = False
        border_points = set()
        for line in self.lines:
            if "$Elements" in line:
            ...
            if in_elem:
                if "$EndElements" in line:
                    break
                parts = line.strip().split()
                if len(parts) > 4 and parts[1] == '1':  # type 1 = Line
                    phys = int(parts[3])
                    if phys == physical_id:
                        # Les deux derniers nombres sont les points de la ligne
                        border_points.update(map(int, parts[-2:]))
        return sorted(border_points)
    def get_surface_points_by_name(self, surface_name):
        # 1. Trouver l'ID physique associé au nom
        physical_id = None
        in_phys = False
        for line in self.lines:
            ...
            if in_phys:
                ...
                if len(parts) >= 3:
                    dim, phys_id, name = int(parts[0]), int(parts[1]), parts[2].strip('"')
                    if dim == 2 and name == surface_name:
                        physical_id = int(phys_id)
                        break
        if physical_id is None:
            print(f"Nom de surface '{surface_name}' non trouvé.")
            return []

        # 2. Trouver les triangles (éléments type 2) associés à cet ID physique
        in_elem = False
        surface_points = set()
        for line in self.lines:
            if "$Elements" in line:
            ...
            if in_elem:
                if "$EndElements" in line:
                    break
                parts = line.strip().split()
                if len(parts) > 4 and parts[1] == '2':  # type 2 = Triangle
                    phys = int(parts[3])
                    if phys == physical_id:
                        # Les trois derniers nombres sont les points du triangle
                        surface_points.update(map(int, parts[-3:]))
        return sorted(surface_points)

    def get_physical_points_by_name(self, point_name):
        # 1. Trouver l'ID physique associé au nom
        physical_id = None
        in_phys = False
        for line in self.lines:
            ...
            if in_phys:
                ...
                if len(parts) >= 3:
                    dim, phys_id, name = int(parts[0]), int(parts[1]), parts[2].strip('"')
                    if dim == 0 and name == point_name:
                        physical_id = int(phys_id)
                        break
        if physical_id is None:
            print(f"Nom de point '{point_name}' non trouvé.")
            return []

        # 2. Trouver les points (éléments type 15) associés à cet ID physique
        in_elem = False
        point_ids = []
        for line in self.lines:
            if "$Elements" in line:
            ...
            if in_elem:
                if "$EndElements" in line:
                    break
                parts = line.strip().split()
                if len(parts) > 4 and parts[1] == '15':  # type 15 = Point
                    phys = int(parts[3])
                    if phys == physical_id:
                        # Le dernier nombre est le numéro du point
                        point_ids.append(int(parts[-1]))
